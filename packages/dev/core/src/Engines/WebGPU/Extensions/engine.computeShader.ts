...
import { ComputeBindingType, type ComputeBindingList, type ComputeBindingMapping, type ComputeCompilationMessages } from "../../Extensions/engine.computeShader";
...
const _GetComputePipelineLayout = (
...
): Nullable<{ layout: GPUPipelineLayout; key: string }> => {
    if (!bindingsMapping) {
        return null;
    }

    let needsExplicitLayout = false;
    ...
    for (const key in bindings) {
        ...
        switch (binding.type) {
            ...
            case ComputeBindingType.InternalTexture: {
                ...
                needsExplicitLayout ||= sampleType === WebGPUConstants.TextureSampleType.UnfilterableFloat || sampleType === WebGPUConstants.TextureSampleType.Depth;
    ...
    return {
        ...
        key: JSON.stringify(bindGroupLayoutEntries),
    };
};
...
WebGPUEngine.prototype._computeDispatch = function (
    ...
    const explicitLayout = _GetComputePipelineLayout(this._device, bindings, bindingsMapping, contextPipeline.sources?.compute ?? "", this._caps.textureFloatLinearFiltering);
    const layoutKey = explicitLayout?.key ?? "auto";

    if (!contextPipeline.computePipeline || contextPipeline.computePipelineLayoutKey !== layoutKey) {
        ...
        contextPipeline.computePipeline = this._device.createComputePipeline({
            layout: explicitLayout?.layout ?? WebGPUConstants.AutoLayoutMode.Auto,
